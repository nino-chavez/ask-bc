...
Ask BC is evolving from a single-tier Next.js app with a classic AI SDK tool loop into a **hybrid architecture** where the agent runtime moves to a Cloudflare Worker built on Project Think + Codemode, while the Next.js UI on Vercel continues to handle OAuth, the iframe shell, App Extensions, and the chat UI. This document describes the **current Vercel-side architecture as it exists today** — the parts that handle auth, routing, UI, and extension registration. The new agent runtime is documented separately:

- [ADR-001: Codemode Agent Runtime on Cloudflare](./decisions/001-codemode-agent-runtime.md) — the architectural decision, motivating gap analysis (versus Cloudflare's Agent Lee), alternatives considered, and consequences
- [Developer guide: `workers/agent-runtime/`](../developer/agent-runtime.md) — operational reference, running locally, known gotchas, and phase plan

Until the migration reaches Phase 2, the chat endpoint on Vercel (`/stores/[hash]/api/chat`) still runs the classic tool loop against the 15 hand-written BC wrappers. The descriptions below reflect that baseline. As the Worker-backed runtime comes online, the sequence diagrams in this document will be amended to show the proxy path.
